...
double calc(map<double,vector<dat> > ma) {
  SCC scc(n+n);
  for(map<double,vector<dat> >::iterator it=ma.begin(); it!=ma.end(); it++) {
    vector<dat> A=it->second;
    for(dat a:A) add_edge(a,scc);
    if(!check(scc)) return -it->first;
  }
    ...
    for(int j=i+1;j<n;j++){
      for(int k=0;k<2;k++)
        for(int l=0;l<2;l++){
          ...
          ma[-cost].push_back((dat){cost,i,j,k,l});
        }
      }
  printf("%.8Lf\n",calc(ma));
